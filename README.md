...
8. [Загрузка файла через табличный документ](docs/ЗагрузкаФайлаЧерезТабличныйДокумент.md)
    * [Обработка универсальной загрузки из файла через табличный документ](docs/УниверсальнаяЗагрузкаИзФайлаЧерезТабличныйДокумент.md)
9. [Переопределения методов БСП](docs/ПереопределениеМетодовБСП.md)
10. [Схемы Запросов](docs/СхемыЗапросов.md)
11. [Подключаемые команды: Настройки формы](docs/ПодключаемыеКомандыНастройкиФормы.md)
    * [Заполнение XDTO-пакета настроек формы](docs/ЗаполнениеXDTOПакетаНастроекФормы.md)

## Внедрение в типовые конфигурации

Для подготовки к внедрению в типовые конфигурации используется обработка ["Первое внедрение"](src/epf). Обработку так же можно использовать для упрощения обновления библиотеки на новые релизы. Подробное описание работы обработки [здесь](docs/ПервоеВнедрение.md)

При внедрении ПБП в конфигурацию на базе БСП, необходимо в модуле БСП "ПодсистемыКонфигурацииПереопределяемый" в процедуре "ПриДобавленииПодсистем" вставить строчку "ОбщийМодуль.ПриДобавленииПодсисте(МодулиПодсистем)".
...
> Разработка ведется на версии 8.3.27
