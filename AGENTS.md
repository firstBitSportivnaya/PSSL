...
- Правило Cursor: `.cursor/rules/psslrule.mdc` (единственный alwaysApply). Фичи Jenkins: `.cursor/rules/vanessa-jenkins.mdc` (`features/**/*.feature`). Skill: `pssl-library`. CodeRabbit CLI: skill `coderabbit-review` (база `develop`, не пользовательский `code-review`). Проверка MCP-сервера: skill `mcp-inspector`, правило `.cursor/rules/mcp-inspector.mdc` (`alwaysApply` выключен; веб и TUI только по явной просьбе).
