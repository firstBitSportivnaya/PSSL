...
- Vanessa: CI — `tools/VBParams.json`. Живой MCP — `tools/va/Start-VanessaMcp.ps1` и `user-VanessaAutomation` (пины `.dev.env`); перед инструментами VA — `mcp_auth`. MCP не `ready` — **не писать `.feature`**. Пакетный Unica `test` `va` — `tools/VBParams.local.json` (gitignore). Не путать с Unica dump/build.
