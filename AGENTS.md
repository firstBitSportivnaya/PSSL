...
- Vanessa: CI — `tools/VBParams.json`. Живой MCP — `tools/va/Start-VanessaMcp.ps1` и `user-VanessaAutomation` (пины `.dev.env`). Пакетный Unica `test` `va` — `tools/VBParams.local.json` (gitignore). Не путать с Unica dump/build.
