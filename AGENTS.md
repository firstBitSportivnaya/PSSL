...
- Unica: `cwd` = `WORKSPACE_ROOT` из `.dev.env`. Пока MCP готов — правки BSL/XML только Unica (`unica_code_patch` и leaf write), не правкой файлов напрямую. Загрузка в ИБ и YaXUnit — `pssl-library` (отдельный `build` на каждый source-set, не `-AllExtensions`). Если `/F` у `1cv8c` совпадает с `INFOBASE_PATH`, закрыть только этот сеанс и продолжать загрузку. Сеансы другой ИБ не завершать.
