...
Процедура ВыполнитьКодСервер()
	...
	ТаблицаПараметров = __ОбщегоНазначенияСервер.КонструкторПараметровПользовательскойФункции();
	...
	Попытка
		Результат = __ОбщегоНазначенияСервер.ВыполнитьПользовательскуюФункциюСПараметрами(Объект.Ссылка, ТаблицаПараметров);
