...
Процедура ЗаполнитьПараметры(Объект) Экспорт
	...
	ИначеЕсли Объект.Контекст = Перечисления.пбп_КонтекстыВыполненияПользовательскихФункций.ФайловаяИнтеграция Тогда
		ДобавитьПараметрыПеременныеФайловаяИнтеграция(Объект);
	КонецЕсли;
...
//  Описание	 - Строка - описание параметра, по умолчанию пустая строка
//
Процедура НайтиДобавитьПараметр(Объект, ИмяПараметра, ТипПараметра = Неопределено, Описание = "")
	
	Если ТипЗнч(ТипПараметра) <> Тип("Строка") Тогда
		ТипСтрокой = ТипСтрокой(ТипПараметра);
	Иначе
		ТипСтрокой = ТипПараметра;
	КонецЕсли;
...
// Выполняет добавляет переменную при отсутствии.
//
// Параметры:
//  Объект				 - СправочникОбъект.пбп_ПользовательскиеФункции - Изменяемый объект
//  ИмяПеременной		 - Строка - Имя добавляемой переменной
//  ЗначениеПеременной	 - Строка - Значение переменной, по умолчанию не заполнено
//  Описание			 - Строка - описание переменной, по умолчанию пустая строка
//
Процедура НайтиДобавитьПеременную(Объект, ИмяПеременной, ЗначениеПеременной = Неопределено, Описание = "")
	
	ДанныеПоиска = Объект.Переменные.НайтиСтроки(
		Новый Структура("Имя", ИмяПеременной));
	
	Если Не ЗначениеЗаполнено(ДанныеПоиска) Тогда
		НоваяСтрока = Объект.Переменные.Добавить();
		НоваяСтрока.Имя			= ИмяПеременной;
...
// Добавляет параметры и переменные, необходимые для контекста Файловый обмен.
//
// Параметры:
//  Объект	 - СправочникОбъект.пбп_ПользовательскиеФункции - Изменяемый объект.
//
Процедура ДобавитьПараметрыПеременныеФайловаяИнтеграция(Объект)
	
	ОписанияПараметровПеременных = ПолучитьОписанияПараметровПеременныхПоФайловымИнтеграциям();
	
	// Параметры
	ПараметрПеременная = "ИнтеграционныйПоток";
	НайтиДобавитьПараметр(
		Объект,
		ПараметрПеременная,
		"СправочникСсылка.пбп_ИнтеграционныеПотоки",
		ОписанияПараметровПеременных[ПараметрПеременная]);
	ПараметрПеременная = "ОбъектыИнтеграции";
	НайтиДобавитьПараметр(
		Объект,
		ПараметрПеременная,
		"ТаблицаЗначений",
		ОписанияПараметровПеременных[ПараметрПеременная]);
	ПараметрПеременная = "ОписаниеОшибки";
	НайтиДобавитьПараметр(
		Объект,
		ПараметрПеременная,
		"Строка",
		ОписанияПараметровПеременных[ПараметрПеременная]);
	ПараметрПеременная = "ПолныйПутьКФайлу";
	НайтиДобавитьПараметр(
		Объект,
		ПараметрПеременная,
		"Строка",
		ОписанияПараметровПеременных[ПараметрПеременная]);
	ПараметрПеременная = "ДатаНачалаИнтеграции";
	НайтиДобавитьПараметр(
		Объект,
		ПараметрПеременная,
		"Дата",
...
Функция ПолучитьОписанияПараметровПеременныхПоФайловымИнтеграциям()
	
	НаименованиеОписание = Новый Структура;
	НаименованиеОписание.Вставить("ИнтеграционныйПоток",
		НСтр("ru = 'Ссылка на интеграционный поток. Заполняется автоматически'"));
	
	НаименованиеОписание.Вставить("ОбъектыИнтеграции",
		НСтр("ru = 'Таблица объектов, участвующих в интеграции с разбивкой по событию: создан / обновлен.
		|Колонки:
		|- ОбъектИнтеграции - ЛюбаяСсылка - ссылка на загружаемый объект
		|- Идентификатор - Строка - идентификатор объекта (уникальное значение)
		|- ТипИдентификатора - Строка - уид, код, номер и т.д.
		|- СозданОбновлен - ПеречислениеСсылка.пбп_СозданОбновлен:
		|    Создан, Обновлен, ОшибкаСоздания, ОшибкаОбновления
		|- ДополнительнаяИнформация - Строка - информация, ошибка и т.д.'"));
	НаименованиеОписание.Вставить("ОписаниеОшибки",
		НСтр("ru = 'Строкое описание ошибки интеграции'"));
	
	НаименованиеОписание.Вставить("ПолныйПутьКФайлу",
		НСтр("ru = 'Полный путь к файлу обмена на сервере'"));
	
	НаименованиеОписание.Вставить("ДатаНачалаИнтеграции",
		НСтр("ru = 'Дата начала интеграции. Заполняется автоматически'"));
	
	Возврат НаименованиеОписание;
	
КонецФункции

#КонецОбласти // Контексты
