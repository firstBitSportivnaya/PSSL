...
#Иначе
	МакетКаркаса = ПолучитьКаркасОбработкиОтладки();
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("epf");
	МакетКаркаса.Записать(ИмяВременногоФайла);
	
	ПутьКИсполняемымФайлам = КаталогПрограммы();
	
	//НаименованиеКаталогаНовойИБ = "БазаДанныхДляВнешнейОбработки";
	//ПутьСозданияНовойИБ = КаталогВременныхФайлов() + НаименованиеКаталогаНовойИБ;
	//Файл = Новый Файл(ПутьСозданияНовойИБ);
	//Если Не Файл.Существует() Тогда
	//	КомандаСозданияИБ = СтрШаблон("1cv8.exe CREATEINFOBASE File=""%1""", ПутьСозданияНовойИБ);
	//	ЗапуститьПриложение(КомандаСозданияИБ, ПутьКИсполняемымФайлам, Истина);
	//КонецЕсли;
	//
	//НаименованиеВложенногоКаталога = "КаркасОбработкиДляОтладкиКода";
	//ПутьВыгрузкиИсходников = КаталогВременныхФайлов() + НаименованиеВложенногоКаталога;
	//НаименованиеКаталогаВыгрузки = ПутьВыгрузкиИсходников + "\" + НаименованиеВложенногоКаталога;
	//ИмяФайлаМодуля = НаименованиеКаталогаВыгрузки + "\Ext\ObjectModule.bsl";
	//
	//Файл = Новый Файл(ПутьВыгрузкиИсходников);
	//Если Не Файл.Существует() Тогда
	//	СоздатьКаталог(ПутьВыгрузкиИсходников);
	//Иначе
	//	УдалитьФайлы(ПутьВыгрузкиИсходников);
	//	СоздатьКаталог(ПутьВыгрузкиИсходников);
	//КонецЕсли;
	//
	//КомандаСохраненияОбработки = СтрШаблон("1cv8.exe DESIGNER /F %1 /DisableStartupMessages /DisableStartupDialogs /DumpExternalDataProcessorOrReportToFiles ""%2"" ""%3""",
	//	ПутьСозданияНовойИБ, ПутьВыгрузкиИсходников, ИмяВременногоФайла);
	//ЗапуститьПриложение(КомандаСохраненияОбработки, ПутьКИсполняемымФайлам, Истина);
	//
	//Файл = Новый Файл(НаименованиеКаталогаВыгрузки);
	//Если Файл.Существует() Тогда
	//	
	//	ТекстовыйФайл = Новый ТекстовыйДокумент;
	//	ТекстовыйФайл.Прочитать(ИмяФайлаМодуля);
	//	
	...
	//	
	//	ТекстовыйФайл.Записать(ИмяФайлаМодуля);
	//	
	//	ИмяВременногоФайлаНовый = ПолучитьИмяВременногоФайла("epf");
	//	КомандаЗагрузкиОбработки = СтрШаблон("1cv8.exe DESIGNER /F %1 /DisableStartupMessages /DisableStartupDialogs /LoadExternalDataProcessorOrReportFromFiles ""%2"" ""%3""",
	//		ПутьСозданияНовойИБ, НаименованиеКаталогаВыгрузки + ".xml", ИмяВременногоФайлаНовый);
	//	ЗапуститьПриложение(КомандаЗагрузкиОбработки, ПутьКИсполняемымФайлам, Истина);
	//	
	//	ДвоичныеДанныеОбработки = Новый ДвоичныеДанные(ИмяВременногоФайлаНовый);
	//	
	//	// TODO: заменить на бсп
	//	УдалитьФайлы(ПутьСозданияНовойИБ);
	//	УдалитьФайлы(ПутьВыгрузкиИсходников);
	//	УдалитьФайлы(ИмяВременногоФайла);
	//	УдалитьФайлы(ИмяВременногоФайлаНовый);
	//	
	//	Результат = ВыполнитьОтладкуОбработки(ДвоичныеДанныеОбработки, СтруктураПараметров);
	//КонецЕсли;
#КонецЕсли
...
Процедура ПодготовитьТекстЗаменыПараметровИПеременныхДляОтладки(СтруктураПараметров, ТекстЗамены)
	...
	Если мЗначенияПараметров.Количество() Тогда
		СтрокаОбъявленияПараметровПеременных = Символы.Таб + "// Параметры";
		Для Каждого КлючИЗначение Из мЗначенияПараметров Цикл
...
&НаСервере
Функция ВыполнитьОтладкуОбработки(ДвоичныеДанныеОбработки, СтруктураПараметров)
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("epf");
	ДвоичныеДанныеОбработки.Записать(ИмяВременногоФайла);
	
	ОбработкаОбъект = ВнешниеОбработки.Создать(ИмяВременногоФайла, БезопасныйРежим());
