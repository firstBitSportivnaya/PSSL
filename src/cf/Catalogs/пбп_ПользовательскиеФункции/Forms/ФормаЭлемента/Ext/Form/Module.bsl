...
&НаКлиенте
Процедура ОтладитьКод(Команда)
	
#Если ВебКлиент Или МобильноеПриложениеКлиент Или МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Отладка не предусмотрена в веб-клиенте и в мобильном приложении.'");
#Иначе
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Действие", "ОбработкаОтладкиПользовательскойФункции");
	
	ОписаниеОповещения = Новый ОписаниеОповещения("СформироватьОбработкуОтладкиПослеЗаполненияПараметров", ЭтотОбъект);
...
&НаСервере
Функция ПолучитьЗначениеИзХранилищаРекурсивно(АдресХранилища)
	
	ПервЗначениеИзХранилища = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	Если ТипЗнч(ПервЗначениеИзХранилища) = Тип("Массив") Тогда
		НоваяКоллекция = Новый Массив;
	ИначеЕсли ТипЗнч(ПервЗначениеИзХранилища) = Тип("Структура") Тогда
		НоваяКоллекция = Новый Структура;
	КонецЕсли;
	
	Для Каждого ЭлементЗначения Из ПервЗначениеИзХранилища Цикл
		
		Если ТипЗнч(ПервЗначениеИзХранилища) = Тип("Массив") Тогда
			Если СтрНачинаетсяС(ЭлементЗначения, "e1cib/tempstorage/") > 0 Тогда
				НоваяКоллекция.Добавить(ПолучитьЗначениеИзХранилищаРекурсивно(ЭлементЗначения));
			Иначе
				НоваяКоллекция.Добавить(ЭлементЗначения);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ПервЗначениеИзХранилища) = Тип("Структура") Тогда
			Если СтрНачинаетсяС(ЭлементЗначения.Значение, "e1cib/tempstorage/") > 0 Тогда
				НоваяКоллекция.Вставить(ЭлементЗначения.Ключ, ПолучитьЗначениеИзХранилищаРекурсивно(ЭлементЗначения.Значение));
			Иначе
				НоваяКоллекция.Вставить(ЭлементЗначения.Ключ, ЭлементЗначения.Значение);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ПервЗначениеИзХранилища) = Тип("ТаблицаЗначений") Тогда
			НоваяКоллекция = ПервЗначениеИзХранилища.Скопировать();
			
		Иначе
			НоваяКоллекция = ПервЗначениеИзХранилища;
...
&НаКлиенте
Асинх Процедура СформироватьОбработкуОтладкиПослеЗаполненияПараметров(
	...
	Если ЗначениеЗаполнено(КаталогВременныхФайлов) Тогда
		...
		ИмяОбработки = "КаркасОтладки";
		ОтносительныйПутьКМодулюФормы = ИмяОбработки + "\Ext\ObjectModule.bsl";
		ПутьКФайлуМодуля = КаталогДляАрхива + ОтносительныйПутьКМодулюФормы;
		
		СтруктураПараметров = Новый Структура;
		Результат = Ждать ОбработатьМодульОбъектаОбработкиОтладкиЗаполнитьПараметры(ПутьКФайлуМодуля, СтруктураПараметров);
		Если Результат = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		АдресОбработкиВХранилище = Ждать ПолучитьПутьКНовойОбработкеОтладки(ПараметрыСохранения,
			КаталогВременныхФайлов, КаталогДляАрхива, ИмяОбработки);
		Если ПараметрыСохранения.Временная Тогда
			УдалитьФайлыНаКлиенте(ПараметрыСохранения.КаталогИнформационнойБазы);
		КонецЕсли;
		
		Если АдресОбработкиВХранилище <> Неопределено Тогда
			Результат = ВыполнитьОтладкуОбработки(АдресОбработкиВХранилище, СтруктураПараметров, КаталогВременныхФайлов);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьИмяФайлаРаспакованногоАрхиваОбработки(КаталогВременныхФайлов)
	...
	АрхивМакет = ПолучитьКаркасОбработкиОтладки();
	...
	АрхивМакет.Записать(ИмяФайлаАрхива);
	
	КаталогДляАрхива = КаталогВременныхФайлов
		+ "КаталогСохраненияОбработкиОтладки" + ПолучитьРазделительПути();
...
&НаКлиенте
Асинх Функция ОбработатьМодульОбъектаОбработкиОтладкиЗаполнитьПараметры(ПутьКФайлуМодуля, СтруктураПараметров)
	
	ТекстовыйФайл = Новый ТекстовыйДокумент;
	
	Попытка
		Ждать ТекстовыйФайл.ПрочитатьАсинх(ПутьКФайлуМодуля);
		СтрокаЗаголовка = "// Создано автоматически в целях отладки пользовательской функции
		|// """ + Объект.Наименование + """
		|//";
		ТекстовыйФайл.ЗаменитьСтроку(1, СтрокаЗаголовка);
		
		ИндексСтроки = 1;
		ИндексСтрокиВозврата = 0;
		Пока ИндексСтрокиВозврата = 0 И ИндексСтроки <= ТекстовыйФайл.КоличествоСтрок() Цикл
			ТекстСтроки = ТекстовыйФайл.ПолучитьСтроку(ИндексСтроки);
			Если СтрНайти(ТекстСтроки, "Возврат") > 0 Тогда
				ИндексСтрокиВозврата = ИндексСтроки;
				Прервать;
			КонецЕсли;
		...
		ТекстЗамены = "";
		ПодготовитьТекстЗаменыПараметровИПеременныхДляОтладки(СтруктураПараметров, ТекстЗамены);
		
		ТекстовыйФайл.ЗаменитьСтроку(ИндексСтрокиВозврата, ТекстЗамены);
		
		Возврат Ждать ТекстовыйФайл.ЗаписатьАсинх(ПутьКФайлуМодуля);
	Исключение
...
&НаКлиенте
Асинх Функция ПолучитьПутьКНовойОбработкеОтладки(ПараметрыСохранения,
	КаталогВременныхФайлов, КаталогДляАрхива, ИмяОбработки)
...
&НаСервере
Функция ПолучитьКаркасОбработкиОтладки()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	МакетКаркаса = ОбработкаОбъект.ПолучитьМакетКаркасаОтладки();
	
	Возврат МакетКаркаса;
	
КонецФункции

&НаКлиенте
Процедура ПодготовитьТекстЗаменыПараметровИПеременныхДляОтладки(СтруктураПараметров, ТекстЗамены)
	...
	Для Каждого Стр Из Объект.ПараметрыФункции Цикл
		СтрокаПараметра = ЗначенияПараметровФункции.Получить(Стр.Идентификатор);
		Значение = ?(СтрокаПараметра = Неопределено, Неопределено, СтрокаПараметра.Значение);
		СтруктураПараметров.Вставить(Стр.Наименование, Значение);
	КонецЦикла;
	
	Если СтруктураПараметров.Количество() Тогда
		СтрокаОбъявленияПараметровПеременных = Символы.Таб + "// Параметры";
		Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
			СтрокаОбъявленияПараметровПеременных = СтрокаОбъявленияПараметровПеременных + Символы.ПС
				+ Символы.Таб + КлючИЗначение.Ключ + " = ПараметрыИПеременные[""" + КлючИЗначение.Ключ + """];";
			
			Если СтрНачинаетсяС(КлючИЗначение.Значение, "e1cib/tempstorage/") > 0 Тогда
				ЗначениеПараметра = ПолучитьЗначениеИзХранилищаРекурсивно(КлючИЗначение.Значение);
			Иначе
				ЗначениеПараметра = КлючИЗначение.Значение;
			КонецЕсли;
			
			СтруктураПараметров.Вставить(КлючИЗначение.Ключ, ЗначениеПараметра);
		КонецЦикла;
		
	ИначеЕсли Объект.ПараметрыФункции.Количество() Тогда
		СтрокаОбъявленияПараметровПеременных = Символы.Таб + "// Параметры";
		Для Каждого СтрокаПараметр Из Объект.ПараметрыФункции Цикл
			СтрокаОбъявленияПараметровПеременных = СтрокаОбъявленияПараметровПеременных + Символы.ПС
				+ Символы.Таб + СтрокаПараметр.Наименование + " = Неопределено;";
		КонецЦикла;
		
	КонецЕсли;
	
	Если Объект.Переменные.Количество() Тогда
		Если ЗначениеЗаполнено(СтрокаОбъявленияПараметровПеременных) Тогда
			СтрокаОбъявленияПараметровПеременных = СтрокаОбъявленияПараметровПеременных
				+ Символы.ПС + Символы.ПС + Символы.Таб + "// Переменные";
		Иначе
			СтрокаОбъявленияПараметровПеременных = Символы.Таб + "// Переменные";
		КонецЕсли;
		
		Для Каждого СтрокаПеременная Из Объект.Переменные Цикл
			СтрокаОбъявленияПараметровПеременных = СтрокаОбъявленияПараметровПеременных + Символы.ПС
				+ Символы.Таб + СтрокаПеременная.Имя + " = ПараметрыИПеременные[""" + СтрокаПеременная.Имя + """];";
			
			СтруктураПараметров.Вставить(СтрокаПеременная.Имя, СтрокаПеременная.Значение);
		КонецЦикла;
	КонецЕсли;
	
	ОтформатированныйТекстПрограммы = СтрЗаменить(Объект.КодПодпрограммы, Символы.ПС, Символы.ПС + Символы.Таб);
	Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
		ОтформатированныйТекстПрограммы = СтрЗаменить(ОтформатированныйТекстПрограммы,
			"Параметры." + КлючИЗначение.Ключ, КлючИЗначение.Ключ);
	КонецЦикла;
	
	ТекстЗамены = "";
	Если ЗначениеЗаполнено(СтрокаОбъявленияПараметровПеременных) Тогда
		ТекстЗамены = СтрокаОбъявленияПараметровПеременных + Символы.ПС + Символы.ПС + Символы.Таб + "// Исполняемый код"
			+ Символы.ПС + Символы.Таб + ОтформатированныйТекстПрограммы;
	Иначе
		ТекстЗамены = ОтформатированныйТекстПрограммы;
	КонецЕсли;
	
	ТекстЗамены = ТекстЗамены + Символы.ПС + Символы.ПС + Символы.Таб + "Возврат Результат;";
	
КонецПроцедуры

&НаСервере
Функция ВыполнитьОтладкуОбработки(АдресОбработкиВХранилище, СтруктураПараметров, КаталогВременныхФайлов)
	...
	ОбработкаОбъект = ВнешниеОбработки.Создать(ИмяВременногоФайла, БезопасныйРежим(), ОписаниеЗащиты);
	Результат = ОтладитьФункцию(ОбработкаОбъект, СтруктураПараметров);
...
&НаСервере
Функция ОтладитьФункцию(ОбработкаОбъект, СтруктураПараметров)
	
	Возврат ОбработкаОбъект.ОтладитьПроизвольнуюФункцию(СтруктураПараметров);
