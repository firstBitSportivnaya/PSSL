...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВывестиДополнительныеНастройки();
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ЗаполнитьНастройкиИзБезопасногоХранилища();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	СохранитьДополнительныеНастройки(ТекущийОбъект);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЭтоТипАвторизацииБезАвторизации(Объект.ТипАвторизации) Тогда
	...
	Если ПараметрыАутентификацииОчищены Тогда

		пбп_Переадресация.УдалитьДанныеИзБезопасногоХранилища(ТекущийОбъект.Ссылка);
		ПараметрыАутентификацииОчищены = Ложь;
...
Процедура ТипИнтеграцииПриИзменении(Элемент)
	...
	УстановитьДоступностьПараметровАутентификации(Элементы, Объект.ТипАвторизации);
	ТипАвторизацииДоИзменения = Объект.ТипАвторизации;
...
&НаКлиенте
Процедура ЭтоТехническаяНастройкаПриИзменении(Элемент)
	УстановитьСвойстваЭлементовВЗависимостиОтТипаИнтеграции();
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьКнопкиПараметрыАутентификации(ЭлементыФормы, ТипАвторизации)
	
	КнопкаДоступна = ТипАвторизации <> ПредопределенноеЗначение("Перечисление.пбп_ТипыАвторизации.БезАвторизации");
	ЭлементыФормы.СписокНастроекДобавитьОбщиеНастройки.Доступность = КнопкаДоступна;
	ЭлементыФормы.СписокНастроекДобавитьОбщиеНастройки.Видимость = КнопкаДоступна;
	
КонецПроцедуры

&НаКлиенте
Процедура ТипАвторизацииПриИзменении(Элемент)
	УстановитьДоступностьКнопкиПараметрыАутентификации(Элементы, Объект.ТипАвторизации);
	...
	Если ЕстьЗаполненныеПараметрыАутентификации() Тогда

		ТекстВопроса = НСтр("ru = 'Параметры аутентификации будут очищены. Продолжить?'");
		Оповещение = Новый ОписаниеОповещения("ТипАвторизацииПриИзмененииЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);

	Иначе

		ЗавершитьИзменениеТипаАвторизации(Истина);
...
Процедура УстановитьСвойстваЭлементов()
	...
	Если Не ЗначениеЗаполнено(Объект.ТипИнтеграции) Тогда
		...
		Элементы.ГруппаНастройкиВыполнения.Видимость = Ложь;
	...
	Элементы.ГруппаОсновныеНастройки.Видимость = ДоступноПоРолям;
	Элементы.СписокНастроек.Видимость = ДоступноПоРолям;
	Элементы.УстановитьСоединениеСБазойДанных.Видимость = ДоступноПоРолям;
	
КонецПроцедуры

&НаКлиенте
// Завершает изменение типа авторизации после ответа пользователя.
Процедура ТипАвторизацииПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт

	ЗавершитьИзменениеТипаАвторизации(РезультатВопроса = КодВозвратаДиалога.Да);

КонецПроцедуры

&НаКлиенте
Процедура ЗавершитьИзменениеТипаАвторизации(ОчиститьПараметры)

	Если ОчиститьПараметры Тогда

		ОчиститьПараметрыАутентификации();

	Иначе

		Объект.ТипАвторизации = ТипАвторизацииДоИзменения;
...
&НаКлиенте
Функция ЕстьЗаполненныеПараметрыАутентификации()
...
&НаКлиенте
Процедура ОчиститьПараметрыАутентификации()
	
	СписокНастроек.Очистить();
	Модифицированность = Истина;
	ПараметрыАутентификацииОчищены = Истина;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьПараметровАутентификации(ЭлементыФормы, ТипАвторизации)

	ПараметрыДоступны = Не ЭтоТипАвторизацииБезАвторизации(ТипАвторизации);
	ЭлементыФормы.СписокНастроек.Доступность = ПараметрыДоступны;
	ЭлементыФормы.СписокНастроекДобавитьОбщиеНастройки.Доступность = ПараметрыДоступны;

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ЭтоТипАвторизацииБезАвторизации(ТипАвторизации)

	Возврат ТипАвторизации =
		ПредопределенноеЗначение("Перечисление.пбп_ТипыАвторизации.БезАвторизации");
...
Процедура УстановитьСвойстваЭлементовВЗависимостиОтТипаИнтеграции()
	...
	Элементы.ГруппаНастройкиВыполнения.Видимость = Не Объект.ЭтоТехническаяНастройка;
	Элементы.ПланОбмена.Видимость = Объект.ЭтоТехническаяНастройка;
...
Функция ПолучитьОбщиеЗначения()
	...
	Массив = Новый Массив;
	Если ЭтоТипАвторизацииБезАвторизации(Объект.ТипАвторизации) Тогда
		Возврат Массив;
	ИначеЕсли Не ЗначениеЗаполнено(Объект.ТипАвторизации)
...
&НаСервере
Процедура ВывестиДополнительныеНастройки()
	
	// Очистка
	УдаляемыеРеквизиты = Новый Массив;
	УдаляемыеЭлементы = Новый Массив;
	Для Каждого ДопНастройка Из СписокДополнительныхНастроек Цикл
		УдаляемыеЭлементы.Добавить(СформироватьИмяДополнительнойНастройки(ДопНастройка.Идентификатор));
		УдаляемыеРеквизиты.Добавить(ДопНастройка.Идентификатор);
	КонецЦикла;
	пбп_РаботаСФормами.УдалитьЭлементы(ЭтотОбъект, УдаляемыеЭлементы);
	пбп_РаботаСФормами.УдалитьРеквизиты(ЭтотОбъект, УдаляемыеРеквизиты);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Идентификатор КАК ИдПараметра,
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Синоним КАК ПараметрПредставление,
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Тип КАК ТипПараметра,
		|	пбп_НастройкиИнтеграцииДополнительныеНастройки.Значение КАК Значение
		|ИЗ
		|	Справочник.пбп_ТипыИнтеграций.ДополнительныеПараметрыНастроекИнтеграций КАК пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.пбп_НастройкиИнтеграции.ДополнительныеНастройки КАК пбп_НастройкиИнтеграцииДополнительныеНастройки
		|		ПО пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Идентификатор = пбп_НастройкиИнтеграцииДополнительныеНастройки.Идентификатор
		|			И (пбп_НастройкиИнтеграцииДополнительныеНастройки.Ссылка = &Ссылка)
		|ГДЕ
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Ссылка = &ТипИнтеграции";
	
	Запрос.УстановитьПараметр("ТипИнтеграции", Объект.ТипИнтеграции);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураСвойств.Вставить("СохраняемыеДанные", Истина);
		
		СтруктураРеквизитов.Вставить(Выборка.ИдПараметра,
			Новый Структура("ОписаниеТипа, СтруктураСвойств", Новый ОписаниеТипов(Выборка.ТипПараметра), СтруктураСвойств));
	КонецЦикла;
	Выборка.Сбросить();
	
	пбп_РаботаСФормами.СоздатьРеквизиты(ЭтотОбъект, СтруктураРеквизитов);
	
	СписокДополнительныхНастроек.Очистить();
	Пока Выборка.Следующий() Цикл
		нСтрока = СписокДополнительныхНастроек.Добавить();
		нСтрока.Идентификатор = Выборка.ИдПараметра;
		
		пбп_РаботаСФормами.СоздатьПоле(ЭтотОбъект,
			СформироватьИмяДополнительнойНастройки(Выборка.ИдПараметра),
			Элементы.ГруппаДополнительныеНастройки,
			Выборка.ПараметрПредставление,
			1,
			Выборка.ИдПараметра);
		
		Если Выборка.Значение <> null Тогда
			ЭтотОбъект[Выборка.ИдПараметра] = Выборка.Значение;
...
&НаСервере
Процедура СохранитьДополнительныеНастройки(Знач ТекущийОбъект)
	
	ТекущийОбъект.ДополнительныеНастройки.Очистить();
	Для Каждого ДопНастройка Из СписокДополнительныхНастроек Цикл
		ЗначениеДопПараметра = ЭтотОбъект[ДопНастройка.Идентификатор];
		
		мСтрок = ТекущийОбъект.ДополнительныеНастройки.НайтиСтроки(
			Новый Структура("Идентификатор", ДопНастройка.Идентификатор));
		Если Не ЗначениеЗаполнено(мСтрок) Тогда
			Стр = ТекущийОбъект.ДополнительныеНастройки.Добавить();
		Иначе
			Стр = мСтрок[0];
		КонецЕсли;
		Стр.Идентификатор = ДопНастройка.Идентификатор;
		Стр.Значение = ЗначениеДопПараметра;
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция СформироватьИмяДополнительнойНастройки(Имя)
	Возврат Имя + "_ДопНастройка";
КонецФункции

#КонецОбласти

&НаСервере
Процедура ЗаполнитьНастройкиИзБезопасногоХранилища()
