...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ Создать реквизиты доп настроек
	...
	Пока Выборка.Следующий() Цикл
		пбп_РаботаСФормами.СоздатьПоле(ЭтотОбъект, Выборка.ИдПараметра, Элементы.ГруппаДополнительныеНастройки, Выборка.ПараметрПредставление, 1, Выборка.ИдПараметра);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Идентификатор КАК Идентификатор
		|ИЗ
		|	Справочник.пбп_ТипыИнтеграций.ДополнительныеПараметрыНастроекИнтеграций КАК пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций
		|ГДЕ
		|	пбп_ТипыИнтеграцийДополнительныеПараметрыНастроекИнтеграций.Ссылка = &ТипИнтеграции";
	...
	Пока Выборка.Следующий() Цикл
		//Попытка
		ЗначениеДопПараметра = ЭтотОбъект[Выборка.Идентификатор];
		
		//Если Не ЗначениеЗаполнено(ЗначениеДопПараметра)
		
		мСтрок = ТекущийОбъект.ДополнительныеНастройки.НайтиСтроки(Новый Структура("Идентификатор", Выборка.Идентификатор));
		...
		Стр.Значение = ЭтотОбъект[Выборка.Идентификатор];
	КонецЦикла;
	// --
