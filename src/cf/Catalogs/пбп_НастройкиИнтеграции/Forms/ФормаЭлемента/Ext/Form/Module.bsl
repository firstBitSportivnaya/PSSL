...
Процедура ТипИнтеграцииПриИзменении(Элемент)
	...
	Объект.ТипАвторизации		= пбп_ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.пбп_ТипыАвторизации.ПустаяСсылка");
	Объект.ИмяОбъекта			= "";
...
Процедура УстановитьСоединениеСБазойДанных(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или Модифицированность Тогда
		ТекстСообщения = НСтр("ru = 'Для изменения параметров соединения с базой данных необходимо записать справочник. Записать?'");
		...
		ИначеЕсли ЭлементНаследования = Справочники.пбп_ТипыИнтеграций.ПрямоеПодключениеКБД Тогда
			...
			Для Каждого ИсточникДанныхЭлемент Из Метаданные.ВнешниеИсточникиДанных Цикл
				Представление = ?(ПустаяСтрока(ИсточникДанныхЭлемент.Синоним),
					ИсточникДанныхЭлемент.Имя, ПустаяСтрока(ИсточникДанныхЭлемент.Синоним));
