...
Процедура ОчиститьИсториюИнтеграции() Экспорт
	...
	КоличествоДнейХранения = пбп_ОбщегоНазначенияСервер.ПолучитьПредопределенноеЗначение(
		"КолДнейХраненияИсторииИнтеграции");
	Если КоличествоДнейХранения = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КоличествоДнейХраненияОшибок = пбп_ОбщегоНазначенияСервер.ПолучитьПредопределенноеЗначение(
