...
Процедура УстановитьСвойстваЭлементов()
	...
	Если Не ЗначениеЗаполнено(Объект.НастройкаИнтеграции) Тогда
		...
		пбп_ПереадресацияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, Метод, УстанавливаемоеСвойство, Ложь);
	Иначе
		РеквизитыОбъекта = пбп_Переадресация.ЗначенияРеквизитовОбъекта(
			Объект.НастройкаИнтеграции, "ТипИнтеграции, ИспользоватьПользовательскиеФункции, ПортВИнтеграционномПотоке");
		...
		ИначеЕсли ЭлементНаследования = Справочники.пбп_ТипыИнтеграций.RESTAPI Тогда
			...
			пбп_ПереадресацияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы, Метод, УстанавливаемоеСвойство, Истина);
		...
		ТипДопускаетПорт = ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.Каталог
			И ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.ПочтовыйКлиент
			И ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.COM
			И ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.ПрямоеПодключениеКБД
			И ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.ВнешняяКомпонента
			И ЭлементНаследования <> Справочники.пбп_ТипыИнтеграций.КоманднаяСтрока;
		
		ПортВиден = РеквизитыОбъекта.ПортВИнтеграционномПотоке И ТипДопускаетПорт;
		
	КонецЕсли;
	
	пбп_ПереадресацияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ХранитьНомераВерсий", УстанавливаемоеСвойство,
		пбп_ОбщегоНазначенияПовтИсп.ПолучитьИспользованиеВерсионированияОбъектовОбмена());
	
	пбп_ПереадресацияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, ПортСтрока, УстанавливаемоеСвойство, ПортВиден);
...
Функция ПроверитьВидимостьНастроекРасписания()
	...
	Если Не ДоступноПоРолям Тогда
		Возврат Видимость;
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.КлючРегламентногоЗадания) Тогда
		Видимость = Истина;
	Иначе
		...
		Видимость = (ЭлементНаследования = Справочники.пбп_ТипыИнтеграций.Каталог
			Или ЭлементНаследования = Справочники.пбп_ТипыИнтеграций.FTPРесурсы
			Или ЭлементНаследования = Справочники.пбп_ТипыИнтеграций.ПочтовыйКлиент)
			И пбп_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеКонстанты(
...
Функция ПолучитьРегламентноеЗадание()
	...
	ОтборЗадания = Неопределено;
	Если Не ПустаяСтрока(Объект.КлючРегламентногоЗадания) Тогда
		ДанныеРеглЗадания = Метаданные.РегламентныеЗадания[Объект.КлючРегламентногоЗадания];
	...
	Если ОтборЗадания = Неопределено Тогда
...
Процедура СоздатьРегламентноеЗадание()
	...
	Если Не ПустаяСтрока(Объект.КлючРегламентногоЗадания) Тогда
		МетаданныеРеглЗадание = Метаданные.РегламентныеЗадания[Объект.КлючРегламентногоЗадания];
		
		ПараметрыЗадания = Новый Структура;
		Если МетаданныеРеглЗадание.Предопределенное Тогда
			НаименованиеРеглЗадания = пбп_ПереадресацияКлиентСервер.ПодставитьПараметрыВСтроку(
				"ru = '%1'", МетаданныеРеглЗадание.Наименование);
		Иначе
			НаименованиеРеглЗадания = пбп_ПереадресацияКлиентСервер.ПодставитьПараметрыВСтроку(
				"ru = '%1: поток %2'", МетаданныеРеглЗадание.Наименование, Объект.Наименование);
		КонецЕсли;
		
		ПараметрыЗадания.Вставить("Наименование"	, НСтр(НаименованиеРеглЗадания));
		ПараметрыЗадания.Вставить("Использование"	, Истина);
		ПараметрыЗадания.Вставить("Метаданные"		, МетаданныеРеглЗадание);

		Если Не МетаданныеРеглЗадание.Предопределенное Тогда
			ПараметрыРегламентногоЗадания = Новый Массив;
			ПараметрыРегламентногоЗадания.Добавить(Объект.Ссылка);
			ПараметрыЗадания.Вставить("Параметры", ПараметрыРегламентногоЗадания);
		КонецЕсли;
	Иначе
		НаименованиеРеглЗадания = пбп_ПереадресацияКлиентСервер.ПодставитьПараметрыВСтроку(
			"ru = 'Выполнение пользовательской функции: выполнение потока ""%1""'",
