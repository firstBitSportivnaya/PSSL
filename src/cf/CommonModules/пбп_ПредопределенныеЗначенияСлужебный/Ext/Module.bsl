...
Функция ДопПараметрыОбработкиПредопределенныхЭлементов(Таблица, МенеджерОбъекта) Экспорт
	...
	Параметры.Вставить("КлючиХешаЭлемент", КлючиХеша.КлючиХешаЭлемент);
	Параметры.Вставить("КлючиХешаГруппа", КлючиХеша.КлючиХешаГруппа);
...
Функция КолонкиПредопределенныхЭлементов(Менеджер) Экспорт
	...
	Колонки = пбп_ПредопределенныеЗначенияПереопределяемый.СтандартныеПоля();
...
// Функция - Добавляемые поля объекта для расчета хеша
//
// Параметры:
//  МенеджерОбъекта	 - 	 - 
// 
...
Функция КолонкиСОписаниемТипов(МенеджерОбъекта, Колонки)
	...
	МетаданныеОбъекта = Метаданные.НайтиПоТипу(ТипОбъекта);//МенеджерОбъекта.ПустаяСсылка().Метаданные();
	...
	НеНайденныеРеквизиты = Новый Массив;
	Для Каждого Колонка Из Колонки Цикл
		...
		Если Реквизит = Неопределено Тогда
			Если Колонка = "Родитель" Тогда
				КолонкиСТипами.Вставить(Колонка, ОписаниеТиповЭлемента);
				Продолжить;
			КонецЕсли;
			
			ЕстьРеквизит = пбп_ПереадресацияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтандартныеРеквизиты, Колонка);
			Если Не ЕстьРеквизит Тогда
				НеНайденныеРеквизиты.Добавить(Колонка);
				Продолжить;
			КонецЕсли;
			
			Реквизит = СтандартныеРеквизиты[Колонка];
			//Если Реквизит = Неопределено Тогда
			//	НеНайденныеРеквизиты.Добавить();
			//	Продолжить;
			//КонецЕсли;
		КонецЕсли;
	...
	//Если ЗначениеЗаполнено(НеНайденныеРеквизиты) Тогда
	//	ВызватьИсключение пбп_ПереадресацияКлиентСервер.ПодставитьПараметрыВСтроку(
	//		НСтр("ru = 'Реквизиты не существуют у объекта: %1'"),
	//		СтрСоединить(НеНайденныеРеквизиты, ", "));
	//КонецЕсли;
