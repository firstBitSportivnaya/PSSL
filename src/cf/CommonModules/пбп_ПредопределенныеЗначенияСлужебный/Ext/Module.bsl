...
Процедура СоздатьПредопределеннуюГруппу(МенеджерОбъекта, СтрокаТаблицы, КлючиХеша, Группы)
	...
	Если Не ЗаполнитьЗначенияЭлемента(МенеджерОбъекта, НоваяГруппа, СтрокаТаблицы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СоздатьПредопределенныйЭлемент(МенеджерОбъекта, СтрокаТаблицы, КлючиХеша, Родители)
	...
	Если Не ЗаполнитьЗначенияЭлемента(МенеджерОбъекта, НовыйЭлемент, СтрокаТаблицы) Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрокаТаблицы.Служебный_Иерархический И ЗначениеЗаполнено(СтрокаТаблицы.ИдентификаторРодитель) Тогда
		Родитель = ПолучитьРодителяЭлемента(Родители, СтрокаТаблицы.Родитель, СтрокаТаблицы.ИдентификаторРодитель);
...
Процедура ОбновитьЭлемент(МенеджерОбъекта, СтрокаТаблицы, КлючиХеша, Родители)
	...
	Если Не ЗаполнитьЗначенияЭлемента(МенеджерОбъекта, Элемент, СтрокаТаблицы) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗаполнитьЗначенияЭлемента(МенеджерОбъекта, Элемент, СтрокаТаблицы)
	
	Свойства = ЗаполняемыеИсключаемыеСвойства(СтрокаТаблицы.Служебный_ИерархияГруппИЭлементов, Элемент.ЭтоГруппа);
	
	РезультатЗаполнения = ЗаполнитьТипЗначенияПВХ(Свойства.ИсключаяСвойства, Элемент, СтрокаТаблицы);
	Если РезультатЗаполнения.ЕстьОшибка Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если РезультатЗаполнения.ЕстьСвойство Тогда
...
Функция ЗаполнитьТипЗначенияПВХ(ИсключаяСвойства, Элемент, СтрокаТаблицы)
	...
	Результат = Новый Структура("ЕстьСвойство, ЕстьОшибка", Ложь, Ложь);
	...
	Мета = Элемент.Ссылка.Метаданные();
	Если Не Мета.Тип.СодержитТип(ПроверяемыйТип) Тогда
