...
// Имена типов из состава типа плана пбп_ПредопределенныеЗначения.
//
// Возвращаемое значение:
//  Массив из Строка - имена для конструктора ОписаниеТипов.
//
Функция ИменаТиповПлана() Экспорт
	
	Результат = Новый Массив;
	ОписаниеТипа = Метаданные.ПланыВидовХарактеристик.пбп_ПредопределенныеЗначения.Тип;
	ИменаПримитивов = СтрРазделить("Строка,Число,Булево,Дата", ",");
	
	Для Каждого ИмяПримитива Из ИменаПримитивов Цикл
		
		Если ОписаниеТипа.СодержитТип(Тип(ИмяПримитива)) Тогда
			
			Результат.Добавить(ИмяПримитива);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого ТипПлана Из ОписаниеТипа.Типы() Цикл
		
		ИмяСсылки = ИмяСсылочногоТипа(ТипПлана);
		
		Если ЗначениеЗаполнено(ИмяСсылки) Тогда
			
			Результат.Добавить(ИмяСсылки);
...
// Имя типа ссылки для конструктора ОписаниеТипов. Для примитива - пустая строка.
//
Функция ИмяСсылочногоТипа(ТипПлана)
	
	МетаТипа = Метаданные.НайтиПоТипу(ТипПлана);
	...
	Префикс = ПрефиксСсылки(Лев(ПолноеИмя, ПозицияТочки - 1));
	...
	Возврат Префикс + "." + Сред(ПолноеИмя, ПозицияТочки + 1);
	
КонецФункции

Функция ПрефиксСсылки(КлассМетаданных)
	
	Префиксы = Новый Соответствие;
	Префиксы.Вставить("Справочник", "СправочникСсылка");
	Префиксы.Вставить("Документ", "ДокументСсылка");
	Префиксы.Вставить("Перечисление", "ПеречислениеСсылка");
	Префиксы.Вставить("ПланВидовХарактеристик", "ПланВидовХарактеристикСсылка");
	Префиксы.Вставить("ПланСчетов", "ПланСчетовСсылка");
	Префиксы.Вставить("ПланВидовРасчета", "ПланВидовРасчетаСсылка");
	Префиксы.Вставить("ПланОбмена", "ПланОбменаСсылка");
	Префиксы.Вставить("БизнесПроцесс", "БизнесПроцессСсылка");
	Префиксы.Вставить("Задача", "ЗадачаСсылка");
	Префикс = Префиксы.Получить(КлассМетаданных);
