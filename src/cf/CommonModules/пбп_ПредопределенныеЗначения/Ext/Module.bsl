...
Процедура ЗаполнениеПредопределенныхЭлементов(ОтправитьСерверноеОповещение = Ложь) Экспорт
	...
	Для Каждого Сеанс Из Сеансы Цикл
		АктивныеСеансыПоКлючам.Вставить(Сеанс.Пользователь, Сеанс);
	КонецЦикла;
	...
	Для Каждого Сеанс Из Сеансы Цикл
		...
		Если пбп_Переадресация.ЭтоПолноправныйПользователь(Сеанс.Пользователь) Тогда
			Если Адресаты.Получить(Сеанс.Пользователь) = Неопределено Тогда
				Адресаты.Вставить(Сеанс.Пользователь.УникальныйИдентификатор,
					пбп_ПереадресацияКлиентСервер.ЗначениеВМассиве(пбп_Переадресация.КлючСеанса(Сеанс)));
			Иначе
				Адресаты[Сеанс.Пользователь.УникальныйИдентификатор].Добавить(пбп_Переадресация.КлючСеанса(Сеанс));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
