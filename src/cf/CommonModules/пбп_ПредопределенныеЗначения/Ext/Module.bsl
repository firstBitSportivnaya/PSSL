...
Функция ИнициализироватьПредопределенныеЗначения(МенеджерОбъекта) Экспорт
	...
	ТЗПредопределенныхЗначений = пбп_ПредопределенныеЗначенияПереопределяемый.ТаблицаПредопределенныхЭлементов(МенеджерОбъекта);
...
//  МенеджерОбъекта			- СправочникМенеджер, ПланВидовХарактеристикМенеджер - менеджер объекта.
//
Функция ОбработатьПредопределенныеЗначения(ТаблицаПредопределенных, МенеджерОбъекта, НачальноеЗаполнение = Ложь) Экспорт
	...
	Для Каждого Строка Из ТаблицаПредопределенных Цикл
		...
		Если ВыборкаДетальныеЗаписи.НайтиСледующий(Строка.ИдентификаторНастройки, "ИдентификаторНастройки") Тогда
			Если ВыборкаДетальныеЗаписи.РучноеИзменение
				Или ВыборкаДетальныеЗаписи.ХешСумма = Строка.ХешСумма Тогда
				СтрокиДляУдаления.Добавить(Строка);
			Иначе
				Строка.Служеб_ПредопределенныйЭлемент = ВыборкаДетальныеЗаписи.Ссылка;
				
				Если ВыборкаДетальныеЗаписи.ХешОтсутствует Тогда
