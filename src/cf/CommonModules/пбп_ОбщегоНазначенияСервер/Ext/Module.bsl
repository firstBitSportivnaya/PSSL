...
Функция ЭтоПримитивныйТип(Знач ПроверяемыйТип) Экспорт
	...
	МассивТипов = Новый ОписаниеТипов("Число, Строка, Булево, Дата, УникальныйИдентификатор, ОписаниеТипов");
	
	Возврат МассивТипов.СодержитТип(ПроверяемыйТип);
...
//
Функция ПолучитьСоответствиеОбъектовИБ(ИдентификаторНастройки, Объект1 = Неопределено,
...
//
Функция ПолучитьЗначенияОбъектаСоответствияПоКлючу(ИдентификаторНастройки,
	ИмяКлюча, ЗначениеКлюча, ИмяОбъекта, ИспользоватьКэш = Ложь) Экспорт
...
// Параметры:
//  ИдентификаторНастройки	 - Строка											 - Идентификатор элемента справочника "Тип соответствия объектов ИБ"
//  Объект1					 - ОпределяемыйТип.пбп_ТипыОбъектовСоответствийИБ	 - Объект1
//  Объект2					 - ОпределяемыйТип.пбп_ТипыОбъектовСоответствийИБ	 - Объект2
//  ИспользоватьКэш			 - Булево											 - признак получения данных из кэша
// 
// Возвращаемое значение:
//  ОпределяемыйТип.пбп_ТипыОбъектовСоответствийИБ - искомое значение по объектам.
// 
Функция ПолучитьЗначениеСоответствияОбъектов(ИдентификаторНастройки,
	Объект1 = Неопределено, Объект2 = Неопределено, ИспользоватьКэш = Ложь) Экспорт
	
	Если ИспользоватьКэш Тогда
		Возврат пбп_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеСоответствияОбъектов(ИдентификаторНастройки, Объект1, Объект2);
	Иначе
...
// Получает соответствие пользователей с полными правами и их сеансов
//
// Возвращаемое значение:
//  Соответствие - Активные сеансы по ключам
//   * Ключ - УникальныйИдентификатор пользователя
//   * Значение - Массив ключей сеансов
//
Функция ПолучитьАдресатовСПолнымиПравами() Экспорт
	...
	Для Каждого Сеанс Из Сеансы Цикл
		...
		Если пбп_Переадресация.ЭтоПолноправныйПользователь(Сеанс.Пользователь) Тогда
			ИдентификаторПользователя = Сеанс.Пользователь.УникальныйИдентификатор;
			Если Адресаты.Получить(ИдентификаторПользователя) = Неопределено Тогда
				Адресаты.Вставить(ИдентификаторПользователя,
					пбп_ПереадресацияКлиентСервер.ЗначениеВМассиве(пбп_Переадресация.КлючСеанса(Сеанс)));
			Иначе
				Адресаты[ИдентификаторПользователя].Добавить(пбп_Переадресация.КлючСеанса(Сеанс));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
