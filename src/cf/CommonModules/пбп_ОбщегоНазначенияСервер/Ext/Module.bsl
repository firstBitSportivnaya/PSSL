...
// Формирует полный текст ошибки и записывает его в журнал регистрации.
// Текст собирается так же, как в ПолучениеПолногоТекстаОшибкиПриИсключении.
//
// Параметры:
//  ИмяСобытия - Строка - имя события журнала регистрации.
//  ЗаголовокОшибки - Строка - описание действия, которое привело к ошибке.
//  ТекстИсключения - Строка - подробное представление ошибки.
//  ДополнительныеСообщения - Массив из СообщениеПользователю - сообщения с реквизитом Текст.
//                                 Если не переданы, в текст ошибки не добавляются.
...
//  Уровень - УровеньЖурналаРегистрации - уровень важности события. По умолчанию Ошибка.
//
Процедура ЗаписатьПолныйТекстОшибкиВЖурналРегистрации(ИмяСобытия, ЗаголовокОшибки, ТекстИсключения,
	ДополнительныеСообщения = Неопределено, Данные = Неопределено, ОбъектМетаданных = Неопределено,
	Уровень = Неопределено) Экспорт
	
	Если ДополнительныеСообщения = Неопределено Тогда
	...
	ТекстОшибки = ПолучениеПолногоТекстаОшибкиПриИсключении(
		ЗаголовокОшибки, ТекстИсключения, ДополнительныеСообщения);
