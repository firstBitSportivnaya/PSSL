...
// Аналог метода БСП. Процедура удаляет из массива МассивРеквизитов элементы, соответствующие именам 
...
Процедура УдалитьНепроверяемыеРеквизитыИзМассива(МассивРеквизитов, МассивНепроверяемыхРеквизитов) Экспорт
	...
	Результат = ПереадресацияМодуляОбщегоНазначения();
	Если Результат.МодульСуществует Тогда
		Результат.Модуль.УдалитьНепроверяемыеРеквизитыИзМассива(МассивРеквизитов, МассивНепроверяемыхРеквизитов);
		Возврат;
	КонецЕсли;
...
// Аналог метода БСП. Возвращает строковое представление типа, например, "СправочникСсылка.ИмяОбъекта", "ДокументСсылка.ИмяОбъекта".
...
Функция СтроковоеПредставлениеТипа(Тип) Экспорт
	...
	Результат = ПереадресацияМодуляОбщегоНазначения();
	Если Результат.МодульСуществует Тогда
		Возврат Результат.Модуль.СтроковоеПредставлениеТипа(Тип);
	КонецЕсли;
...
// Аналог метода БСП. Определяет принадлежность объекта метаданных к ссылочному типу.
//
// Параметры:
...
// Аналог метода БСП. Проверка того, что переданный тип является ссылочным типом данных.
...
Функция ЭтоСсылка(ПроверяемыйТип) Экспорт
	...
	Результат = ПереадресацияМодуляОбщегоНазначения();
	Если Результат.МодульСуществует Тогда
		Возврат Результат.Модуль.ЭтоСсылка(ПроверяемыйТип);
	КонецЕсли;
	
	Возврат ПроверяемыйТип <> Тип("Неопределено") И пбп_ПереадресацияПовтИсп.ОписаниеТипаВсеСсылки().СодержитТип(ПроверяемыйТип);
	
КонецФункции

#Область Запросы

// Аналог метода БСП. Возвращает фрагмент текста запроса, отделяющего один запрос от другого.
//
// Возвращаемое значение:
...
Функция РазделительПакетаЗапросов() Экспорт
	...
	Результат = ПереадресацияМодуляОбщегоНазначения();
	Если Результат.МодульСуществует Тогда
		Возврат Результат.Модуль.РазделительПакетаЗапросов();
	КонецЕсли;
...
// Аналог метода БСП. Создает полную копию структуры, соответствия, массива, списка или таблицы значений, рекурсивно,
...
// Аналог метода БСП. Для процедуры ПриДобавленииСерверныхОповещений общего модуля ОбщегоНазначенияПереопределяемый.
//
// Параметры:
...
Функция НовоеСерверноеОповещение(Имя) Экспорт
	...
	РезультатВычисления = ПереадресацияМодуляСерверныеОповещения();
	Если РезультатВычисления.МодульСуществует Тогда
		Возврат РезультатВычисления.Модуль.НовоеСерверноеОповещение(Имя);
	КонецЕсли;
	
	Источник = ПланыВидовХарактеристик.пбп_ПредопределенныеЗначения;
	
	ПериодПроверкиСостояния = пбп_ПредопределенныеЗначения.ПредопределенныйЭлемент(Источник,
		"ПериодПроверкиСостоянияСервераОповещенийВСекундах");
	ПериодПроверкиПоУмолчанию = пбп_ПредопределенныеЗначения.ЗначениеПредопределенногоЭлемента(Источник,
		ПериодПроверкиСостояния);
		
	Если ПериодПроверкиПоУмолчанию = Неопределено Тогда
		ПериодПроверкиПоУмолчанию = 20*60;
	КонецЕсли;
	...
	Результат.Вставить("ПериодПроверки", ПериодПроверкиПоУмолчанию);
...
// Аналог метода БСП. Возвращает строковый уникальный ключ сеанса,
...
Функция КлючСеанса(Сеанс = Неопределено) Экспорт
	...
	РезультатВычисления = ПереадресацияМодуляСерверныеОповещения();
	Если РезультатВычисления.МодульСуществует Тогда
		Возврат РезультатВычисления.Модуль.КлючСеанса(Сеанс);
	КонецЕсли;
...
// Аналог метода БСП. Проверяет, является ли текущий или указанный пользователь полноправным.
// 
...
Функция ЭтоПолноправныйПользователь(Пользователь = Неопределено,
	...
	Результат = ПереадресацияМодуляПользователи();
	Если Результат.МодульСуществует Тогда
		Возврат Результат.Модуль.ЭтоПолноправныйПользователь(Пользователь,
...
// Аналог метода БСП. Для функций ЭтоПолноправныйПользователь и РолиДоступны.
// Описание
//
// Параметры:
...
Функция СвойстваПроверяемогоПользователяИБ(Пользователь) Экспорт
	...
	Результат = ПереадресацияМодуляПользователи();
	Если Результат.МодульСуществует Тогда
		Возврат Результат.Модуль.СвойстваПроверяемогоПользователяИБ(Пользователь);
	КонецЕсли;
	
	СвойстваТекущегоПользовательИБ = пбп_ПереадресацияПовтИсп.СвойстваТекущегоПользователяИБ();
	ПользовательИБ = Неопределено;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Пользователь) Тогда
			ИдентификаторПользователяИБ = пбп_Переадресация.ЗначениеРеквизитаОбъекта(Пользователь, "ИдентификаторПользователяИБ");
