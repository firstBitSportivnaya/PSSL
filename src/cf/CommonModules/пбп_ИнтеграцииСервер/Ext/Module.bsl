...
Функция ПроверитьФайлЕстьНаSFTP(ИнтеграционныйПоток,
	ИмяФайла, КаталогПоиска = "/", ПоТочномуСовпадению = Истина) Экспорт
...
Функция ПолучитьМассивФайловСодержимогоSFTPСервера(ПутьКФайлуСодержимого)
	...
	Пока ТекущаяСтрока <> Неопределено Цикл
		Если Лев(ТекущаяСтрока, 1) = "-" Тогда
			ЗаполнитьИмяФайлаВМассивИзСтрокиСпискаФайлов(МассивФайлов, ТекущаяСтрока, НужноеКоличество);
		КонецЕсли;
...
Процедура ЗаполнитьИмяФайлаВМассивИзСтрокиСпискаФайлов(МассивФайлов, ТекущаяСтрока, НужноеКоличество)
	
	// Разделяем по времени создания
	МассивВремя = пбп_СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекущаяСтрока, ":");
	Если МассивВремя.Количество() = НужноеКоличество Тогда
