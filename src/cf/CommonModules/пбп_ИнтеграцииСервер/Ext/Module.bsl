...
//
Процедура УдалитьСоответствиеОбъектовИнтегрируемыхСистем(ИнтегрируемаяСистема,
	СсылкаНаОбъект = Неопределено, ИдентификаторСистемы = Неопределено) Экспорт
...
#Область ТокеныИнтегрируемыеСистем

// Получает токены интегрируемой системы
//
// Параметры:
//  ИнтегрируемаяСистема - СправочникСсылка.пбп_ИнтегрируемыеСистемы - интегрируемая система
...
//
Функция ПолучитьТокеныИнтегрируемойСистемы(ИнтегрируемаяСистема, Организация = Неопределено,
	Пользователь = Неопределено, ВнешнийПользователь = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(ИнтегрируемаяСистема) Тогда
		ВызватьИсключение НСтр("ru='Не указана интегрируемая система'");
	КонецЕсли;
	
	Возврат РегистрыСведений.пбп_ТокеныИнтегрируемыхСистем.ПолучитьЗаписи(
		ИнтегрируемаяСистема, Организация, Пользователь, ВнешнийПользователь);
	
КонецФункции

// Создать изменить токены интегрируемой системы
//
// Параметры:
//  ИнтегрируемаяСистема - СправочникСсылка.пбп_ИнтегрируемыеСистемы	 - интегрируемая система
//  Сущность			 - ПеречислениеСсылка.пбп_СущностиАвторизации	 - токен, код, секрет и пр.
//  Значение			 - Строка										 - значение токена, кода, секрета и пр.
//  СрокЖизниДо			 - Дата											 - дата до которой действует токен
//  Организация			 - ОпределяемыйТип.пбп_Организации				 - организация
//  Пользователь		 - ОпределяемыйТип.пбп_Пользователи				 - пользователь
//  ВнешнийПользователь	 - Строка										 - пользователь внешней системы
//  Изменение			 - Булево										 - изменение или создание
//
Процедура СоздатьИзменитьТокеныИнтегрируемойСистемы(ИнтегрируемаяСистема, Сущность, Значение, СрокЖизниДо = Неопределено,
	Организация = Неопределено, Пользователь = Неопределено, ВнешнийПользователь = "", Изменение = Ложь) Экспорт
	
	Если Не ЗначениеЗаполнено(ИнтегрируемаяСистема) Тогда
		ВызватьИсключение НСтр("ru='Не указана интегрируемая система'");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Сущность) Тогда
		ВызватьИсключение НСтр("ru='Не указана сущность авторизации'");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Значение) Тогда
		ВызватьИсключение НСтр("ru='Не указана значение сущности авторизации'");
	КонецЕсли;
	
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("ИнтегрируемаяСистема"	, ИнтегрируемаяСистема);
	ПараметрыЗаписи.Вставить("Сущность"				, Сущность);
	ПараметрыЗаписи.Вставить("Значение"				, Значение);
	Если ЗначениеЗаполнено(СрокЖизниДо) Тогда
		ПараметрыЗаписи.Вставить("СрокЖизниДо", СрокЖизниДо);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВнешнийПользователь) Тогда
		ПараметрыЗаписи.Вставить("ВнешнийПользователь", ВнешнийПользователь);
	КонецЕсли;
	
	Если Изменение Тогда
		РегистрыСведений.пбп_ТокеныИнтегрируемыхСистем.ИзменитьЗапись(ПараметрыЗаписи);
	Иначе
		РегистрыСведений.пбп_ТокеныИнтегрируемыхСистем.ДобавитьЗапись(ПараметрыЗаписи);
	КонецЕсли;
	
КонецПроцедуры

// Удалить токены интегрируемой системы
//
// Параметры:
//  ИнтегрируемаяСистема - СправочникСсылка.пбп_ИнтегрируемыеСистемы	 - интегрируемая система
//  Организация			 - ОпределяемыйТип.пбп_Организации				 - организация
...
//  Сущность			 - ПеречислениеСсылка.пбп_СущностиАвторизации	 - токен, код, секрет и пр.
//
Процедура УдалитьТокеныИнтегрируемойСистемы(ИнтегрируемаяСистема, Организация = Неопределено,
	Пользователь = Неопределено, ВнешнийПользователь = Неопределено, Сущность = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Сущность) Тогда
		ПараметрыЗаписи.Вставить("Сущность", Сущность);
	КонецЕсли;
	
	РегистрыСведений.пбп_ТокеныИнтегрируемыхСистем.УдалитьЗаписи(ПараметрыЗаписи);
	
КонецПроцедуры

#КонецОбласти // ТокеныИнтегрируемыеСистем
...
// См. пбп_ИнтеграцииСлужебный.ПолучитьJWT
//
Функция ПолучитьJWT(Данные, Секрет, Алгоритм = "HS256") Экспорт
	
	Возврат пбп_ИнтеграцииСлужебный.ПолучитьJWT(Данные, Секрет, Алгоритм);
	
КонецФункции

// См. пбп_ИнтеграцииСлужебный.Base64UrlEncode
//
Функция Base64UrlEncode(СтрокаBase64) Экспорт
	
	Возврат пбп_ИнтеграцииСлужебный.Base64UrlEncode(СтрокаBase64);
	
КонецФункции

// См. пбп_ИнтеграцииСлужебный.HMACSHA
//
Функция HMACSHA(ДанныеСтрока, СекретСтрока, ТипАлгоритма = Неопределено, КодировкаТекста = "UTF-8") Экспорт
	
	Возврат пбп_ИнтеграцииСлужебный.HMACSHA(ДанныеСтрока, СекретСтрока, ТипАлгоритма, КодировкаТекста);
...
Функция ПрочитатьЗаголовкиСоставногоЗапроса(Чтение)
	...
	Пока Истина Цикл
		...
		ИмяЗаголовка = НРег(СокрЛП(Части[0]));
