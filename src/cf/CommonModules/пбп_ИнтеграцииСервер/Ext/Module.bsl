...
//  ИмяЧастиТела			 - Строка		 - имя части сообщения, где находится текст запроса
...
Функция КонвертироватьЗапросТипаMultipartFormData(Запрос,
	СимволыРазделителя = "--", ИмяЧастиТела = "", ПоместитьФайлыВХранилище = Истина,
	...
	Попытка
		РезультатЧтения = ПолучитьДанныеОбъектовИзСоставногоЗапроса(Запрос,
			СимволыРазделителя, ИмяЧастиТела, ПоместитьФайлыВХранилище, СтруктураИстории);
...
Функция ПолучитьДанныеОбъектовИзСоставногоЗапроса(Запрос,
	...
	РезультатЧтения = ПрочитатьСообщениеСоставногоЗапроса(Заголовки, Тело,
		СимволыРазделителя, ИмяЧастиТела, ПоместитьФайлыВХранилище);
...
Функция ПрочитатьСообщениеСоставногоЗапроса(Заголовки, Тело,
	СимволыРазделителя, ИмяЧастиТела, ПоместитьФайлыВХранилище)
	...
	Для Каждого Буфер Из БуферыДвоичныхДанных Цикл
		...
		// Если имя части тела не указано явно, то подразумевается, что в первой части находится текст сообщения
		Если (Не ПустаяСтрока(ИмяЧастиТела) И ИменаЧасти.name = ИмяЧастиТела) Или ПерваяИтерация Тогда
			ТекстСообщения = ЧтениеЧасти.ПрочитатьСимволы();
		Иначе
			Если ИменаЧасти.Свойство("filename") Тогда
				ИмяЧасти = ИменаЧасти.filename;
			Иначе
				ИмяЧасти = ИменаЧасти.name;
			КонецЕсли;
