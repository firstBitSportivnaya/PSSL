...
Функция ПрочитатьСообщениеСоставногоЗапроса(Заголовки, Тело,
	...
	Для Каждого Буфер Из БуферыДвоичныхДанных Цикл
		...
		ИмяЧасти = ПолучитьИмяСообщенияСоставногоЗапроса(ЗаголовкиЧасти, ПерваяИтерация);
		
		// Если имя части тела не указано явно, то подразумевается, что в первой части находится текст сообщения
		Если (Не ПустаяСтрока(ИмяЧастиТела) И ИмяЧасти = ИмяЧастиТела) Или ПерваяИтерация Тогда
...
Функция ПолучитьИмяСообщенияСоставногоЗапроса(Заголовки, ПерваяИтерация)
	...
	Если ПерваяИтерация Тогда
		ЗначениеСравнения = "name";
	Иначе
		ЗначениеСравнения = "filename";
	КонецЕсли;
	...
	Для Каждого Свойство Из Свойства Цикл
		...
		Если ИмяСвойства = ЗначениеСравнения Тогда
			Имя = СокрЛП(Части[1]);
			Имя = СтрЗаменить(Имя, """", "");
		КонецЕсли;
	КонецЦикла;
