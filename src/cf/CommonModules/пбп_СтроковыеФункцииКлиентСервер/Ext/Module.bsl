...
Функция ПреобразоватьДатуИзСтрокиПоФормату(ВхЗначение, НомерФормата) Экспорт
	...
	Если НомерФормата = "0_0_0" Тогда
		...
		#Иначе
		// В веб-клиенте XMLЗначение недоступна, используем альтернативный способ парсинга ISO даты
		Попытка
			...
			Если СтрНайти(ДатаВремяСтрока, "T") > 0 Тогда
				...
				Год = СтрокаВЧисло(ЧастиДаты[0], 0, 0);
				Месяц = СтрокаВЧисло(ЧастиДаты[1], 0, 0);
				День = СтрокаВЧисло(ЧастиДаты[2], 0, 0);
				
				Часы = СтрокаВЧисло(ЧастиВремени[0], 0, 0);
				Минуты = СтрокаВЧисло(ЧастиВремени[1], 0, 0);
				...
				Если СтрНайти(СекундыСтрока, ".") > 0 Тогда
					...
					Секунды = СтрокаВЧисло(ЧастиСекунды[0], 0, 0);
				Иначе
					Секунды = СтрокаВЧисло(СекундыСтрока, 0, 0);
				КонецЕсли;
