...
Функция ПреобразоватьДатуИзСтрокиПоФормату(ВхЗначение, НомерФормата) Экспорт
	...
	Если НомерФормата = "0_0_0" Тогда
		...
		Попытка
			СтрокаДаты = НормализоватьСтрокуДатыISO(Строка(ВхЗначение));
		...
		Попытка
			ДатаВремяСтрока = НормализоватьСтрокуДатыISO(Строка(ВхЗначение));
			Если СтрНайти(ДатаВремяСтрока, "T") > 0 Тогда
				...
				Если ЧастиДатаВремя.Количество() >= 2 Тогда
					...
					Если ЧастиДаты.Количество() >= 3 И ЧастиВремени.Количество() >= 2 Тогда
						Успешно = Истина;
						Год = БезопасноПреобразоватьСтрокуВЧисло(ЧастиДаты[0], Успешно);
						Месяц = БезопасноПреобразоватьСтрокуВЧисло(ЧастиДаты[1], Успешно);
						День = БезопасноПреобразоватьСтрокуВЧисло(ЧастиДаты[2], Успешно);
						Часы = БезопасноПреобразоватьСтрокуВЧисло(ЧастиВремени[0], Успешно);
						Минуты = БезопасноПреобразоватьСтрокуВЧисло(ЧастиВремени[1], Успешно);
						СекундыСтрока = ?(ЧастиВремени.Количество() >= 3, ЧастиВремени[2], "0");
						Секунды = БезопасноПреобразоватьСтрокуВЧисло(СекундыСтрока, Успешно);
						Если Успешно Тогда
	...
	Иначе
		...
		Иначе
			...
			Если МассивРазделителейДатаВремя.Количество() = 0 Тогда
				Возврат ПреобразованнаяДата;
			КонецЕсли;
		...
		УспешноКомпонентов = Истина;
		ГодЧисло = БезопасноПреобразоватьСтрокуВЧисло(Год, УспешноКомпонентов);
		МесяцЧисло = БезопасноПреобразоватьСтрокуВЧисло(Месяц, УспешноКомпонентов);
		ДеньЧисло = БезопасноПреобразоватьСтрокуВЧисло(День, УспешноКомпонентов);
		...
		Если ЗначениеЗаполнено(МинутыСекунды) Тогда
			...
			Если МассивСекМин.Количество() = 0 Тогда
				МинутыЧисло = 0;
				СекундыЧисло = 0;
			Иначе
				МинутыЧисло = БезопасноПреобразоватьСтрокуВЧисло(МассивСекМин[0], УспешноКомпонентов);
				Если МассивСекМин.Количество() > 1 Тогда
					СекундыЧисло = БезопасноПреобразоватьСтрокуВЧисло(МассивСекМин[1], УспешноКомпонентов);
				Иначе
					СекундыЧисло = 0;
				КонецЕсли;
			КонецЕсли;
		...
		ЧасыЧисло = БезопасноПреобразоватьСтрокуВЧисло(Часы, УспешноКомпонентов);
		
		Если Не УспешноКомпонентов Тогда
...
// Отсекает смещение зоны, Z и миллисекунды в хвосте ISO-даты после T.
Функция НормализоватьСтрокуДатыISO(СтрокаДаты)
	
	ПозицияТ = СтрНайти(СтрокаДаты, "T");
	Если ПозицияТ = 0 Тогда
		Возврат СтрокаДаты;
	КонецЕсли;
	
	ПозицияПлюс = СтрНайти(СтрокаДаты, "+", , ПозицияТ);
	Если ПозицияПлюс > 0 Тогда
		СтрокаДаты = Лев(СтрокаДаты, ПозицияПлюс - 1);
	Иначе
		ПозицияМинусВоВремени = СтрНайти(Сред(СтрокаДаты, ПозицияТ + 1), "-");
		Если ПозицияМинусВоВремени > 0 Тогда
			СтрокаДаты = Лев(СтрокаДаты, ПозицияТ + ПозицияМинусВоВремени - 1);
		КонецЕсли;
	КонецЕсли;
	
	ПозицияZ = СтрНайти(ВРег(СтрокаДаты), "Z", , ПозицияТ);
	Если ПозицияZ > 0 Тогда
		СтрокаДаты = Лев(СтрокаДаты, ПозицияZ - 1);
	КонецЕсли;
	
	ПозицияТочки = СтрНайти(СтрокаДаты, ".", , ПозицияТ);
...
// Безопасное преобразование строки в число (в т.ч. для веб-клиента).
Функция БезопасноПреобразоватьСтрокуВЧисло(СтрокаЗначение, Успешно = Неопределено)
