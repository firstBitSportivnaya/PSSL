...
//
Функция ПолучитьПараметрыПодключенияFTPSFTP(НастройкаИнтеграции, ИнтеграционныйПоток = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ИнтеграционныйПоток) Тогда
		СтруктураНастроек = пбп_ИнтеграцииСлужебный.ПолучитьСтруктуруПотокаИНастроекИнтеграции(ИнтеграционныйПоток);
	Иначе
	...
	Если ПараметрыСоединения.Порт < 0 Или ПараметрыСоединения.Порт > 65535 Тогда
		ВызватьИсключение НСтр("ru = 'Указан недопустимый порт подключения. Допустимы значения 0 и диапазон 1-65535.'");
	КонецЕсли;
...
Функция ВыполнитьПодключениеКFTPСерверуДляИнтеграционногоПотока(ИнтеграционныйПоток) Экспорт
	...
	ПараметрыСоединения = ПолучитьПараметрыПодключенияFTPSFTP(
		СтруктураРеквизитов.НастройкаИнтеграции, ИнтеграционныйПоток);
...
Функция ПолучитьТекстСкриптаОтправкиCURL(ПараметрыСоединения, ПутьОтправляемогоФайла, ПутьФайлаНаСервере)
	...
	|
	|curl -k ""sftp://%1:%2%3"" --user ""%4:%5"" -T %6";
	
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес, Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПутьФайлаНаСервере,
...
Функция ПолучитьТекстСкриптаСодержимогоКаталоговCURL(ПараметрыСоединения, ПутьКФайлуСодержимогоКаталога)
	...
	ТекстСкрипта = "#!/usr/bin/bash
	|
	|curl -k ""sftp://%1:%2%3"" --user ""%4:%5"" > %6";
	
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес, Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПараметрыСоединения.КаталогИсходящие,
...
Функция ПолучитьТекстСкриптаПолученияCURL(ПараметрыСоединения, ПутьСохраняемогоФайла, ПутьФайлаНаСервере)
	...
	ТекстСкрипта = "#!/usr/bin/bash
	|
	|curl -k ""sftp://%1:%2%3"" --user ""%4:%5"" -o %6";
...
Функция ПолучитьТекстСкриптаУдаленияФайлаCURL(ПараметрыСоединения, ПолныйПутьКФайлу)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес,
		Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПараметрыСоединения.Логин,
...
Функция ПолучитьТекстСкриптаОтправкиSSHPASS(ПараметрыСоединения, ПутьОтправляемогоФайла, ПутьФайлаНаСервере)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес, ПараметрыСоединения.Логин,
		ПараметрыСоединения.Пароль, Формат(ПараметрыСоединения.Порт, "ЧГ=0"),
		ПутьОтправляемогоФайла, ПутьФайлаНаСервере);
...
Функция ПолучитьТекстСкриптаСодержимогоКаталоговSSHPASS(ПараметрыСоединения, ПутьКФайлуСодержимогоКаталога)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес, ПараметрыСоединения.Логин,
		ПараметрыСоединения.Пароль, Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПараметрыСоединения.КаталогИсходящие,
...
Функция ПолучитьТекстСкриптаПолученияSSHPASS(ПараметрыСоединения, ПутьСохраняемогоФайла, ПутьФайлаНаСервере)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес, ПараметрыСоединения.Логин,
		ПараметрыСоединения.Пароль, Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПутьФайлаНаСервере, ПутьСохраняемогоФайла);
...
Функция ПолучитьТекстСкриптаУдаленияФайлаSSHPASS(ПараметрыСоединения, ПолныйПутьКФайлу)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, ПараметрыСоединения.Адрес,
		...
		Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПолныйПутьКФайлу);
...
Функция ПолучитьТекстСкриптаОтправкиEXPECT(ПараметрыСоединения, ПутьОтправляемогоФайла, ПутьФайлаНаСервере)
	...
	ТекстСкрипта = СтрШаблон(ТекстСкрипта, Формат(ПараметрыСоединения.Порт, "ЧГ=0"), ПараметрыСоединения.Логин,
