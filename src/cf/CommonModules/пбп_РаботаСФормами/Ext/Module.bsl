...
//
Функция ОписаниеТипаСтрока(ДлинаСтроки = 0, ДопустимаяДлинаСтроки = Неопределено) Экспорт
	
	Возврат Новый ОписаниеТипов("Строка",, КвалификаторСтрока(ДлинаСтроки, ДопустимаяДлинаСтроки));
...
//
//  ЗнакЧисла - ДопустимыйЗнак - допустимый знак. По умолчанию любое число.
Функция ОписаниеТипаЧисло(ДлинаЧисла = 15, ТочностьЧисла = 2, ЗнакЧисла = Неопределено) Экспорт
	
	Возврат Новый ОписаниеТипов("Число", КвалификаторЧисло(ДлинаЧисла, ТочностьЧисла, ЗнакЧисла));
...
//  ДлинаСтроки - Число - длина строки
//
Функция КвалификаторСтрока(ДлинаСтроки = 0, ДопустимаяДлинаСтроки = Неопределено) Экспорт
...
//
//  ЗнакЧисла - ДопустимыйЗнак - допустимый знак. По умолчанию любое число.
Функция КвалификаторЧисло(ДлинаЧисла = 15, ТочностьЧисла = 2, ЗнакЧисла = Неопределено) Экспорт
	...
	Возврат Новый КвалификаторыЧисла(ДлинаЧисла, ТочностьЧисла, ЗнакЧисла);
...
//  СтруктураСвойств - Структура, Неопределено		 - Передается структура свойство реквизита формы.
//
Процедура СоздатьРеквизит(Форма, ИмяРеквизита, ОписаниеТипа, СтруктураСвойств = Неопределено) Экспорт
	
	ВложеннаяСтруктура = Новый Структура("ОписаниеТипа, СтруктураСвойств", ОписаниеТипа, СтруктураСвойств);
	
	СтруктураРеквизитов = Новый Структура;
	СтруктураРеквизитов.Вставить(ОчиститьОтЗапрещенныхСимволов(ИмяРеквизита), ВложеннаяСтруктура);
...
// Параметры:
//  Форма				 - ФормаКлиентскогоПриложения	 - Форма на которой создаются элементы
//  СтруктураРеквизитов	 - Структура					 - Где ключ это Имя реквизита,
//  					а значение вложенная структура содержащая ключи ОписаниеТипа, СтруктураСвойств.
//
Процедура СоздатьРеквизиты(Форма, СтруктураРеквизитов) Экспорт
	...
	Для Каждого ТекущийРеквизит Из СтруктураРеквизитов Цикл
		...
		Если ТипЗнч(ТекущийРеквизит.Значение) = Тип("Структура") Тогда
			НовыйРеквизит = НовыйРеквизитСоСвойствами(ТекущийРеквизит);
		Иначе
...
// Создание реквизитов таблиц по списку
//
// Параметры:
//  Форма				 - ФормаКлиентскогоПриложения - форма, на которой создаются элементы.
//  ТаблицыИРеквизиты	 - Соответствие - ключ: имя таблицы, значение: структура реквизитов,
//										где ключ - имя реквизита, значение - ОписаниеТипа.
//
// Пример:
//  ТаблицыИРеквизиты = Новый Соответствие;
//
//  РеквизитыТовары = Новый Структура;
//  РеквизитыТовары.Вставить("Размер", Новый ОписаниеТипов("Число"));
//  РеквизитыТовары.Вставить("Описание", Новый ОписаниеТипов("Строка"));
//  ТаблицыИРеквизиты.Вставить("Объект.Товары", РеквизитыТовары);
//
//  РеквизитыУслуги = Новый Структура;
//  РеквизитыУслуги.Вставить("Назначение", Новый ОписаниеТипов("Строка"));
//  ТаблицыИРеквизиты.Вставить("Объект.Услуги", РеквизитыУслуги);
//
//  СоздатьРеквизитыТаблиц(ЭтаФорма, ТаблицыИРеквизиты);
//
Процедура СоздатьРеквизитыТаблиц(Форма, ТаблицыИРеквизиты) Экспорт
	
	ДобавляемыеРеквизиты = Новый Массив;
	
	Для Каждого Элемент Из ТаблицыИРеквизиты Цикл
...
// Создание реквизитов для таблицы на основании колонок таблицы значений
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - Форма на которой создаются элементы 
//	ИмяТаблицы - Строка - Имя таблицы куда добавляете реквизиты
//	КолонкиТаблицы - КоллекцияКолонокТаблицыЗначений - Колонки из таблицы значений по которой нужно создать реквизиты формы
//
Процедура СоздатьРеквизитыТаблицыПоКолонкам(Форма, ИмяТаблицы, КолонкиТаблицы) Экспорт
	
	ДобавляемыеРеквизиты = Новый Массив;
	Для Каждого Колонка Из КолонкиТаблицы Цикл
		НовыйРеквизит = Новый РеквизитФормы(Колонка.Имя, Колонка.ТипЗначения, ИмяТаблицы, Колонка.Заголовок);
		ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	КонецЦикла;
...
// Создает дерево соответствия описания реквизитов формы (в т.ч. реквизитов объекта и табличных частей)
//
// Параметры:
//  Форма	 - ФормаКлиентскогоПриложения - форма
// 
// Возвращаемое значение:
//  Структура - Ключ - имя, Значение - тип реквизита.
//
//  Для объекта, таблицы формы, табличной часть объекта создается вложенная структура.
//  При наличии реквизитов формы, реквизитов объекта, табличной части объекта и таблицы формы
//  иерархия будет выглядеть следующим образом (ключ - значение):
//    1) РеквизитФормы1 - Строка;
//    2) Объект - Структура:
//      2.1) РеквизитОбъекта1 - Число;
//      2.2) РеквизитОбъекта2 - Структура (это табличная часть):
//        2.2.1) РеквизитКолонкаТЧ1 - Булево;
//    3) РеквизитФормы2 - Структура (это табличная часть):
//      3.1) РеквизитКолонкаТЧ1 - Дата.
//
Функция ПолучитьСвойстваРеквизитовФормыВСтруктуру(Форма) Экспорт
	
	МетаданныеОбъекта = ПолучитьМетаданныеОбъектаПоФорме(Форма.ИмяФормы);
	
	СвойстваРеквизитов = Новый Структура;
	
	РеквизитыФормы = Форма.ПолучитьРеквизиты();
	Для Каждого РеквизитСвойства Из РеквизитыФормы Цикл
		Если ТипЗнч(РеквизитСвойства) = Тип("ОбъектМетаданных") Тогда
			ТипОписание = РеквизитСвойства.Тип;
		Иначе
			ТипОписание = РеквизитСвойства.ТипЗначения;
		КонецЕсли;
		
		Если РеквизитСвойства.Имя = "Объект" Тогда
			СвойстваРеквизитов.Вставить(РеквизитСвойства.Имя, Новый Структура);
			
			Для Каждого РеквизитОбъекта Из МетаданныеОбъекта.Реквизиты Цикл
				СвойстваРеквизитов[РеквизитСвойства.Имя].Вставить(
					РеквизитОбъекта.Имя, РеквизитОбъекта.Тип.Типы()[0]);
			КонецЦикла;
			
			Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
				СвойстваРеквизитов[РеквизитСвойства.Имя].Вставить(
					ТабличнаяЧасть.Имя, Новый Структура);
					
				Для Каждого РеквизитТаблицы Из ТабличнаяЧасть.Реквизиты Цикл
					СвойстваРеквизитов[РеквизитСвойства.Имя][ТабличнаяЧасть.Имя].Вставить(
						РеквизитТаблицы.Имя, РеквизитТаблицы.Тип.Типы()[0]);
				КонецЦикла;
			КонецЦикла;
		ИначеЕсли ТипОписание = Новый ОписаниеТипов("ТаблицаЗначений") Тогда
			СвойстваРеквизитов.Вставить(РеквизитСвойства.Имя, Новый Структура);
			
			ТаблицаЗначений = Форма[РеквизитСвойства.Имя].Выгрузить();
			Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
				СвойстваРеквизитов[РеквизитСвойства.Имя].Вставить(
					Колонка.Имя, Колонка.ТипЗначения.Типы()[0]);
			КонецЦикла;
		Иначе
			СвойстваРеквизитов.Вставить(РеквизитСвойства.Имя, РеквизитСвойства.ТипЗначения.Типы()[0]);
		КонецЕсли;
	КонецЦикла;
...
// Получает объект метаданных по имени формы
//
// Параметры:
//  ИмяФормы - Строка - имя формы
// 
// Возвращаемое значение:
//  ОбъектМетаданных - найденный объект метаданных
//
Функция ПолучитьМетаданныеОбъектаПоФорме(ИмяФормы) Экспорт
	...
	ИмяКлассаВЕдЧисле = ЧастиИмениФормы[0];
	ИмяОбъекта = ЧастиИмениФормы[1];
	
	Если ИмяКлассаВЕдЧисле = "ПланОбмена" Тогда
		МенеджерМетаданных = Метаданные.ПланыОбмена;
	ИначеЕсли ИмяКлассаВЕдЧисле = "Справочник" Тогда
		МенеджерМетаданных = Метаданные.Справочники;
	ИначеЕсли ИмяКлассаВЕдЧисле = "Документ" Тогда
		МенеджерМетаданных = Метаданные.Документы;
	ИначеЕсли ИмяКлассаВЕдЧисле = "Отчет" Тогда
		МенеджерМетаданных = Метаданные.Отчеты;
	ИначеЕсли ИмяКлассаВЕдЧисле = "Обработка" Тогда
		МенеджерМетаданных = Метаданные.Обработки;
	ИначеЕсли ИмяКлассаВЕдЧисле = "ПланВидовХарактеристик" Тогда
		МенеджерМетаданных = Метаданные.ПланыВидовХарактеристик;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат МенеджерМетаданных[ИмяОбъекта];
	
КонецФункции

#КонецОбласти // РеквизитыФормы
...
Функция НовыйРеквизитСоСвойствами(ТекущийРеквизит)
	
	Если Не ТекущийРеквизит.Значение.Свойство("ОписаниеТипа") Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не указан тип для создаваемого реквизита %1';
			|en = 'The type for the created prop is not specified %1'"),
			ТекущийРеквизит.Ключ);
	КонецЕсли;
	
	НовыйРеквизит = Новый РеквизитФормы(ТекущийРеквизит.Ключ, ТекущийРеквизит.Значение.ОписаниеТипа);
	
	Если ТекущийРеквизит.Значение.Свойство("СтруктураСвойств")
		И ТипЗнч(ТекущийРеквизит.Значение.СтруктураСвойств) = Тип("Структура") Тогда
		Для Каждого Свойство Из ТекущийРеквизит.Значение.СтруктураСвойств Цикл
			НовыйРеквизит[Свойство.Ключ] = Свойство.Значение;
		КонецЦикла;
	КонецЕсли;
