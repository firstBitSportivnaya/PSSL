...
//
Функция ПолучитьПромежуточнуюТаблицуИзCSVФайла(ПутьКФайлу, СтруктураМакета, Кодировка = "UTF8", Разделитель = "", ЕстьЗаголовок = Истина) Экспорт
	...
	Пока ТекущаяСтрока <> Неопределено Цикл
		Если НомерСтроки < 2
			И ЕстьЗаголовок Тогда // в первой заголовок
	...
	Для Каждого СтрокаТаблицы Из ТЗ Цикл
		...
		Если КонвертироватьДаты Тогда
			Для Каждого КлючИЗначение Из СтруктураИменКолонокДат Цикл
				СтрокаТаблицы[КлючИЗначение.Ключ] = пбп_СтроковыеФункцииКлиентСервер.ПреобразоватьДатуИзСтрокиПоФормату(
...
Функция ПолучитьНомерФорматаДатыИзСтроки(ЗначениеКолонки)
	...
	Если ЕстьВремя Тогда
		...
		ИначеЕсли МассивРазделителейДатаВремя.Количество() = 0 Тогда
			...
			КонецЕсли;
		Иначе
			ТолькоДата = пбп_СтроковыеФункцииКлиентСервер.ПолучитьТолькоДатуЕслиРазделительПробел(МассивРазделителейДатаВремя);
		КонецЕсли;
	Иначе
