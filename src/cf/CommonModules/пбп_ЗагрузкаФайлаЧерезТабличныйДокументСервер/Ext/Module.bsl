...
Функция ИнициализироватьТаблицуСоСвойствамиКолонок() Экспорт
	...
	СвойстваКолонок.Колонки.Добавить("ОператорУсловия"		, пбп_Переадресация.ОписаниеТипаСтрока(ДлинаОператора));
	СвойстваКолонок.Колонки.Добавить("ФункцияПреобразования", пбп_Переадресация.ОписаниеТипаСтрока(300));
	СвойстваКолонок.Колонки.Добавить("ФорматДаты"			, пбп_Переадресация.ОписаниеТипаСтрока(ДлинаОператора));
	// BSLLS:MagicNumber-on
	
	ДобавитьПоляДляПоискаВТаблицуСвойствКолонок(СвойстваКолонок, ДлинаСтроки, ДлинаОператора);
...
// Заполняет таблицу свойств колонок загружаемого файла из макета конфигурации
//
// Параметры:
//  Макет			 - Макет - макет с описанием колонок, см. макет 
//    Обработка.пбп_УниверсальнаяЗагрузкаИзФайлаЧерезТабличныйДокумент.СтруктураКолонокФайла
//  ТаблицаСвойств	 - ТаблицаЗначений - таблица, в которую будут заполнены свойства колонок,
//    см. ИнициализироватьТаблицуСоСвойствамиКолонок
//
Процедура ЗаполнитьТаблицуСвойствИзМакета(Макет, ТаблицаСвойств) Экспорт
	
	ПостроительОтчета = Новый ПостроительОтчета;
	ПостроительОтчета.ИсточникДанных = Новый ОписаниеИсточникаДанных(
		Макет.Область(1, 1, Макет.ВысотаТаблицы, Макет.ШиринаТаблицы));
	ПостроительОтчета.Выполнить();

	ВыборкаРезультата = ПостроительОтчета.Результат.Выбрать();

	Пока ВыборкаРезультата.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаСвойств.Добавить(), ВыборкаРезультата);
	КонецЦикла;
...
Функция ПреобразоватьСтроковыеЗначенияПоТипамСвойств(ТЗ, Макет, КонвертироватьДаты = Ложь,
		...
		Если СтрНайти(СтрокаТаблицы.ТипЗначения, СсылкаСтрока) <> 0
			...
			Если Не СтрНайти(ВставкаИсточники, " " + ИмяОбъекта + " ") > 0 И ЕстьПоляПоиска Тогда
				...
				Для Инкремент = 1 По 3 Цикл
					...
					ОператорСравнения = СтрокаТаблицы["Оператор" + Инкремент];
					Если ПустаяСтрока(ОператорСравнения) Тогда
						ОператорСравнения = "=";
					КонецЕсли;
					
					ПредопределенноеЗначение = СтрокаТаблицы["Значение" + Инкремент];
					Если ПустаяСтрока(ПредопределенноеЗначение) Тогда
						ИсходноеЗначение = ВходнаяТаблицаСтрока + ИмяКолонкиСравнения;
						ПроверкаНаПустоеЗначение = ИсходноеЗначение + " <> """" И " + ИсходноеЗначение + " <> 0 И ";
						СравниваемоеЗначение = ИмяОбъекта + Итерация + "." + РеквизитСравнения;
					Иначе
						ИсходноеЗначение = ИмяОбъекта + Итерация + "." + РеквизитСравнения;
						ОператорУсловия = "И";
						ПроверкаНаПустоеЗначение = "";
						СравниваемоеЗначение = ПредопределенноеЗначение;
					КонецЕсли;
					
					ВставкаУсловияСоединения = ВставкаУсловияСоединения
						+ ?(ПустаяСтрока(ВставкаУсловияСоединения), "", Символы.ПС + "
						|		" + ВРег(ОператорУсловия) + " ") + ПроверкаНаПустоеЗначение
						+ ИсходноеЗначение + " " + ОператорСравнения + " " + СравниваемоеЗначение;
				КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из ТЗ Цикл
		...
		Для Каждого ИмяКолонкиБулево Из МассивИменКолонокБулево Цикл
			ЗначениеДляПроверки = НРег(СтрокаТаблицы[ИмяКолонкиБулево + "Строкой"]);
			
			Если ПустаяСтрока(ЗначениеИстина) Тогда
				НайденоПоЗначению = Ложь;
			Иначе
				НайденоПоЗначению = ЗначениеДляПроверки = НРег(ЗначениеИстина);
			КонецЕсли;
			
			СтрокаТаблицы[ИмяКолонкиБулево] = НайденоПоЗначению
				Или ЗначениеДляПроверки = "да"
				Или ЗначениеДляПроверки = "истина"
				Или ЗначениеДляПроверки = "yes";
		КонецЦикла;
...
Процедура ДобавитьПоляДляПоискаВТаблицуСвойствКолонок(СвойстваКолонок, ДлинаСтроки, ДлинаОператора, КоличествоВсего = 3)
	
	Для Инкремент = 1 По КоличествоВсего Цикл
		СвойстваКолонок.Колонки.Добавить("ПолеПоиска" + Инкремент	, пбп_Переадресация.ОписаниеТипаСтрока(ДлинаСтроки));
		СвойстваКолонок.Колонки.Добавить("Оператор" + Инкремент		, пбп_Переадресация.ОписаниеТипаСтрока(ДлинаОператора));
		СвойстваКолонок.Колонки.Добавить("Значение" + Инкремент		, пбп_Переадресация.ОписаниеТипаСтрока(ДлинаСтроки));
	КонецЦикла;
...
Процедура ПрисвоитьНомераФорматовКолонокДат(ТЗ, СтруктураИменКолонокДат)
	...
	// Ищем значение в первых 30 строках, так как не все значения могут быть заполнены.
	ИскатьЗаполненнуюДатуДо = 29;
	КоличествоСтрокПроверки = Мин(ТЗ.Количество() - 1, ИскатьЗаполненнуюДатуДо);
	Для Каждого КлючИЗначение Из СтруктураИменКолонокДат Цикл
		...
		ЗначениеКолонки = "";
		Для Инкремент = 0 По КоличествоСтрокПроверки Цикл
			ЗначениеКолонки = ТЗ[Инкремент][КлючИЗначение.Ключ + "Строкой"];
			Если Не ПустаяСтрока(ЗначениеКолонки) Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		// Если значение не найдено, то игнорируем поле
...
//   - 3 - "-";
//   - 4 - " ";
//   - 5 - без разделителей. Формат не определяется автоматически, должен быть указан в параметрах колонок
//  Второй разряд принимает значения:
//   - 0 - месяц, день, год. Формат не определяется автоматически, должен быть указан в параметрах колонок
