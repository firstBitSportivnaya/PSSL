...
// Перенаправляет экспортную процедуру в ОбщийМодуль.ВыполнитьМетодКонфигурации(), если в конфигурации есть БСП,
// перенаправляет на метод БСП, если нет выполняется аналог метода БСП, встроенный в конфигурацию.
//
// Параметры:
...
Процедура ВыполнитьМетодКонфигурации(Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Иначе
		__ОбщегоНазначенияСлужебный.ВыполнитьМетодКонфигурации(ИмяМетода, Параметры);
	КонецЕсли;
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	Если Данные <> Неопределено Тогда
		...
		Если БезопасноеХранилищеДанных.Выбран() Тогда
			...
			Иначе
				__ОбщегоНазначенияСлужебныйКлиентСервер.ДополнитьСтруктуру(ДанныеДляСохранения, Данные, Истина);
			КонецЕсли;
...
Функция ПрочитатьДанныеИзБезопасногоХранилища(Владелец, Ключи = "Пароль", ОбщиеДанные = Неопределено) Экспорт
	...
	Владельцы = __ОбщегоНазначенияСлужебныйКлиентСервер.ЗначениеВМассиве(Владелец);
...
Функция ПрочитатьДанныеВладельцевИзБезопасногоХранилища(Владельцы, Ключи = "Пароль", ОбщиеДанные = Неопределено) Экспорт
	...
	__ОбщегоНазначенияСлужебныйКлиентСервер.Проверить(ТипЗнч(Владельцы) = Тип("Массив"),
		__СтроковыеФункцииСлужебныйКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение параметра %1 в %2.
...
Процедура СообщитьПользователю(Знач ТекстСообщенияПользователю, Знач КлючДанных = Неопределено,	Знач Поле = "",
	...
	Сообщение = __ОбщегоНазначенияКлиентСервер.СообщениеПользователю(ТекстСообщенияПользователю,
...
Функция ДанныеИзБезопасногоХранилища(Владельцы, Ключи, ОбщиеДанные)
	...
	ИмяБезопасногоХранилищаДанных = "РегистрСведений.__БезопасноеХранилищеДанных";
