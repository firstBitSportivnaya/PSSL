...
Перед VA-инструментами всегда **`mcp_auth`** на `user-VanessaAutomation` (пустые arguments), даже если порт уже слушает. Если namespace `error`/`needsAuth` или порт не слушает `1cv8c`: **один** Shell `tools/va/Start-VanessaMcp.ps1` (без Bypass). После старта ждать `ready`: до **3** повторных `mcp_auth` с паузой (~10 с), без повторного запуска скрипта. Если MCP всё ещё не `ready` — **стоп**: не писать/править `.feature`, не угадывать шаги (токены + непредсказуемый Gherkin). Пользователь поднимает Vanessa / MCP Restart. Прогон фич — `run_scenario`, не Unica `test` `va`. Не `unica launch mcp-va` applied. Не использовать Vanessa MCP для фактов дампа XML (это Unica).
